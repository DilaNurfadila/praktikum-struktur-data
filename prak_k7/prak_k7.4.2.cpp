...
void DspAll_Del_FilterData(char *namafile, int flag)
{
	string inp;
	system("clear");
	...
	else
	{
		while (myfile.read((char *)&barang, sizeof(barang)) && !myfile.eof())
		{
			...
			else if (barang.flag && flag == 2 && strstr(barang.nama, inp.c_str()))
			{
				system("clear");
				cout << "Nama Barang : ";
				getline(cin, inp);
...
void ModifyData(char *namafile)
{
	...
	else
	{
		...
		else
		{
			...
			cout << "Data barang yang baru" << endl;
			cout << "=====================" << endl;
			cout << "Kode  : ";
			getline(cin, inp);
			strcpy(barang.kode, inp.c_str());
...
void Recvry_DelData(char *namafile, bool flag)
{
	long pos;
	string inp;
	system("clear");

	cout << "Kode barang yang akan di undelete : ";
	getline(cin, inp);
	...
	else
	{
		...
		{
			cout << "Data barang yang akan di-recovery" << endl;
			cout << "=============================" << endl;
			cout << "Kode  : " << barang.kode << endl;
			cout << "Nama  : " << barang.nama << endl;
			cout << "Harga : " << barang.harga << endl;
			...
			cout << "Tekan y untuk undelete...";
			if (getch() != 'y')
			{
				cout << endl
					 << "Data tidak jadi diundelete..." << endl;
			}
			else
			{
				barang.flag = flag;
...
int main()
{
	...
	do
	{
		...
		case '1':
			...
			break;
		case '2':
			DspAll_Del_FilterData(namafile, 1);
			break;
		case '3':
			DspAll_Del_FilterData(namafile, 2);
			break;
		case '4':
			ModifyData(namafile);
			break;
		case '5':
			Recvry_DelData(namafile, 0);
			break;
		case '6':
			DspAll_Del_FilterData(namafile, 0);
			break;
		case '7':
			Recvry_DelData(namafile, 1);
			break;
		case '8':
